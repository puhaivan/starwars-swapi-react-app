  ...
  return (
    <header className="w-full py-8 px-4 bg-gradient-to-r from-black via-gray-900 to-black shadow-md border-b">
      <div className="max-w-4xl mx-auto text-center">
        ...
        <div ref={dropdownRef} className="relative inline-block text-left mt-6">
          <button
          ...
          {isOpen && (
            <div onMouseLeave={() => setIsOpen(false)} className="absolute z-10 mt-2 w-48 rounded-md shadow-lg bg-white/90 ring-1 ring-black/5 backdrop-blur-md">
              <ul className="py-1">
                {pages.map((page) => (
                  <li key={page.key}>
                    <button
        ...
        <div className="mt-4">
          <input
            ...
            className="mt-2 px-4 py-2 rounded bg-white/20 text-yellow-100 placeholder-yellow-300 focus:outline-none focus:ring-2 focus:ring-yellow-400"
          />
        </div>
      </div>
    </header>
